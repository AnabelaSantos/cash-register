function cashRegister(price, cash, cid) {
  ...
  if (cidTotal < changeDueStart) {
    ...
    //case 4a and b
    //loop through change and remove zero values
    else {
      let change2 = [];
      for (let i in change) {
        if (change[i][1] != 0) {
          change2.push([change[i][0], change[i][1]]);
        }
      }
      message.status = "OPEN";
      message.change = change2.reverse();
      return message;
    }
  }
}
